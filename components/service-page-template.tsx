...
export function ServicePageTemplate({
  ...
  const handleBookService = (serviceId: string, subServiceName: string) => {
    setSelectedService(`${subServiceName} - ${serviceId}`);
    setIsBookingModalOpen(true);
  };

  const getServiceRouteId = (name: string) => {
    ...
    for (const [n, id] of Object.entries(serviceMap)) {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Local header (kept simple; global Navigation already shows auth state) */}
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden border-t border-border bg-background">
            <nav className="container mx-auto px-4 py-4 space-y-4">
              <Link href="/services" className="block text-foreground hover:text-primary transition-colors py-2" onClick={() => setIsMobileMenuOpen(false)}>
                Services
              </Link>
              <Link href="/about" className="block text-foreground hover:text-primary transition-colors py-2" onClick={() => setIsMobileMenuOpen(false)}>
                About
              </Link>
              <Link href="/portfolio" className="block text-foreground hover:text-primary transition-colors py-2" onClick={() => setIsMobileMenuOpen(false)}>
                Portfolio
              </Link>
              <Link href="/contact" className="block text-foreground hover:text-primary transition-colors py-2" onClick={() => setIsMobileMenuOpen(false)}>
                Contact
              </Link>
        ...
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-center">
            ...
            <div className="relative order-first lg:order-last">
              <img src={heroImage || "/placeholder.svg"} alt={`${serviceName} professional service`} className="rounded-2xl shadow-2xl w-full h-auto" />
                        ...
                        <div className="space-y-2">
                          {service.issues.slice(0, 4).map((issue, index) => (
                            <div key={index} className="flex items-center space-x-2">
                              ...
                              <label htmlFor={`${service.id}-issue-${index}`} className="text-sm text-muted-foreground cursor-pointer">
                                {issue}
                              </label>
                            </div>
                          ))}
                  ...
                  <CardHeader>
                    <div className="flex items-center space-x-1 mb-2">
                      {[...Array(t.rating)].map((_, j) => (
                        <Star key={j} className="h-4 w-4 text-accent fill-current" />
                      ))}
                    </div>
                    <CardTitle className="font-heading text-base md:text-lg">{t.name}</CardTitle>
                    <Badge variant="secondary" className="w-fit text-xs">
                      {t.service}
                    </Badge>
                  </CardHeader>
                  <CardContent>
                    <p className="text-sm md:text-base text-muted-foreground italic">"{t.comment}"</p>
                  </CardContent>
                </Card>
              ))}
            </div>
      ...
      <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-40">
        <Button size="lg" className="rounded-full w-12 h-12 md:w-14 md:h-14 bg-accent hover:bg-accent/90 shadow-lg" asChild>
      ...
      <BookingModal isOpen={isBookingModalOpen} onClose={() => setIsBookingModalOpen(false)} serviceName={serviceName} preSelectedService={getServiceRouteId(serviceName)} />
    </div>
  );
}
